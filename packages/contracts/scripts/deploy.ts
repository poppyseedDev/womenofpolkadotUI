...
const main = async () => {
  ...
  let { abi, wasm } = await getDeploymentData('collection')
  const collection = await deployContract(api, account, abi, wasm, 'default', [])

  // Write contract addresses to `{contract}/{network}.ts` file(s)
  await writeContractAddresses(chain.network, { collection })
}
